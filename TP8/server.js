...
app.post('/files', upload.array('myFile'), function(req, res){
    ...
    files.forEach(a => {
        let quarantinePath = __dirname + '/' + req.files[a].path
        let newPath = __dirname + '/public/fileStore/' + req.files[a].originalname
        ...
        fs.rename(quarantinePath, newPath, function(err){
            if(err){
                ...
                files.push({
                    date: d,
                    name: req.files[a].originalname,
                    mimetype: req.files[a].mimetype,
                    size: req.files[a].size
                })
